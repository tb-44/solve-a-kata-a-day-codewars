...
//If the input is anything else "the dog" ate the cookie. The way to return the statement is: "Who ate the last cookie? 
//It was (name)!"
...
//solution
function cookie(x) {
    let message = (typeof x == 'string') ? 'Zach' :
        (typeof x == 'number') ? 'Monica' : 'the dog';
    return "Who ate the last cookie? " + "It was " + message + "!";
}
...
function cookie(x) {
    ...
    if (typeof x === 'string') result = 'Zach';
    else if (typeof x === 'number') result = 'Monica';
    else result = 'the dog';
