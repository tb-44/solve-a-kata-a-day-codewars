...
function weight(number) {
    return number.split('').reduce((s, t) => s + Number(t), 0)
}
...
function closest(strng) {
    return strng === '' ? [] :
        strng
            .split(' ')
            .map((x, i) => [weight(x), i, Number(x)])
            .sort((a, b) => { return a[0] !== b[0] ? a[0] - b[0] : a[1] - b[1] })
            .map((x, y, a) => { return y === 0 ? {} : { com: [a[y - 1], x], d: x[0] - a[y - 1][0] }})
            .slice(1)
            .sort((u, v) => u.d != v.d ? u.d - v.d :
                u.com[0][0] != v.com[0][0] ? u.com[0][0] - v.com[0][0] : u.com[0][1] - v.com[0][1])
        [0].com;
}