...
// If you are attempting this Kata in NASM then note that the code example shown
// directly above may not be relevant; please refer to the Sample Tests (written
// in C) for the exact definition of the Node structure.
...
// article on Wikipedia. Specifically, the linked lists this Kata is referring
// to are singly linked lists, where the value of a specific node is stored in
// its data/$data/Data property, the reference to the next node is stored in its
// next/$next/Next/next_node property and the terminator for a list is
// null/NULL/None/nil/nullptr/null().

// Task
// If you are attempting this Kata in NASM, the code examples shown below may
// not be relevant at all - please refer to the Sample Tests (written in C) for
...
// representation of a list must always end with
// null/NULL/None/nil/nullptr/null() (all caps or all lowercase depending on the
// language you are undertaking this Kata in). For example, given the following
// list:
...
// "null"/"NULL"/"None"/"nil"/"nullptr"/@"NULL"/"null()" (again, depending on
// the language).
